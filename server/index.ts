    ...
    if (hits.length === 0) {
      return {
        content: [
          {
            type: "text",
            text: "Keine Treffer im CCTP-Wiki.",
          },
        ],
      };
    }
...
server.tool(
  ...
  async ({ bereich }) => {
    return {
      content: [{ type: "text", text: JSON.stringify(listWikiPages(bereich), null, 2) }],
    };
  },
);
...
server.tool(
  ...
  "Schreibt eine neue Wiki-Seite (oder überschreibt eine bestehende) unter wiki/<bereich>/<dateiname>.md. " +
    ...
    "das Werkzeug lehnt diese Status ab. " +
  ...
  async ({ bereich, dateiname, inhalt, ueberschreiben }) => {
    const result = schreibeWikiSeite({ bereich, dateiname, inhalt, ueberschreiben });
    if (!result.ok) {
      return { content: [{ type: "text", text: `Fehler: ${result.fehler}` }], isError: true };
    }
    return {
      content: [{ type: "text", text: `Seite geschrieben: ${result.pfad}` }],
    };
  },
);
